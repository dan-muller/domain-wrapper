...
import { DefaultErrorShape, defaultFormatter, ErrorFormatter, ErrorFormatterShape } from "./error";
...
class TRPCBuilder<TParams extends PartialRootConfigTypes = object> {
  ...
  create<TOptions extends Partial<RuntimeConfig<CreateRootConfigTypesFromPartial<TParams>>>>(
    options?: ValidateShape<TOptions, Partial<RuntimeConfig<CreateRootConfigTypesFromPartial<TParams>>>> | undefined
  ) {
...
function createTRPCInner<TParams extends PartialRootConfigTypes>() {
  ...
  return function initTRPCInner<TOptions extends $Runtime>(runtime?: ValidateShape<TOptions, $Runtime>) {
